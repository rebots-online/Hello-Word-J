...
export class TextParsingService { // Renamed class
  ...
  private applySubstitutions(lines: string[], substitutions: string): string[] {
    // Simple line selection:
    // :N (select line N)
    // :N-M (select lines N to M)
    // :!N (select all but line N)
    // :!N-M (select all but lines N to M)
    // Regex substitution s/pattern/replacement/flags is NOT handled here yet.

    const lineSelectionMatch = substitutions.match(/^!?(\d+)(?:-(\d+))?$/);

    if (lineSelectionMatch) {
      const isNegated = substitutions.startsWith('!');
      const startLine = parseInt(lineSelectionMatch[1], 10);
      const endLine = lineSelectionMatch[2] ? parseInt(lineSelectionMatch[2], 10) : startLine;

      // Adjust to 0-based indexing for arrays
      const startIndex = startLine - 1;
      const endIndex = endLine - 1;

      if (startIndex < 0 || startIndex >= lines.length || endIndex < 0 || endIndex >= lines.length || startIndex > endIndex) {
        console.warn(`TextParsingService: Invalid line selection range in substitution: ${substitutions} for ${lines.length} lines. Returning original.`);
        return lines;
      }

      if (isNegated) {
        return lines.filter((_, index) => index < startIndex || index > endIndex);
      } else {
        return lines.slice(startIndex, endIndex + 1);
      }
    }
    // TODO: Implement regex s/pattern/replacement/flags if needed.
    console.warn(`TextParsingService: Substitution format '${substitutions}' not fully implemented or unknown. Returning original lines.`);
    return lines;
  }
        ...
        for (const line of lines) {
          ...
          if (includeMatch) {
            ...
            if (includedFileContent) {
              let sectionsOfIncludedFile = this.fileCache.get(`${currentLang}/${includeFullFile}`)?.parsedSections;
              if (!sectionsOfIncludedFile) {
                  sectionsOfIncludedFile = this.parseSections(includedFileContent);
                  const cacheEntry = this.fileCache.get(`${currentLang}/${includeFullFile}`);
                  if(cacheEntry) cacheEntry.parsedSections = sectionsOfIncludedFile;
              }
              const sectionToIncludeLinesOriginal = sectionsOfIncludedFile.get(inclSection);

              if (sectionToIncludeLinesOriginal) {
                let linesToProcess = sectionToIncludeLinesOriginal;
                if (inclSubstitutions) { // Apply substitutions if any
                  linesToProcess = this.applySubstitutions(sectionToIncludeLinesOriginal, inclSubstitutions);
                }

                // console.log(`Including ${currentLang}/${includeFullFile} section [${inclSection}] into ${currentFilePath} [${sectionName}] (Lines: ${linesToProcess.length})`);
                for (const includedLine of linesToProcess) {
